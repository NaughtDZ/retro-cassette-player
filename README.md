...
- **练习册播放列表**：奶白纸面 + 蓝色格线 + 双红线页边 + 行号「1.」+ 红色 ▶ 三角 + 当前曲目荧光笔高亮，
  双击任意行播放，标题栏可拖动，默认置顶。
- **皮肤包（磁带可分层）**：底座 / 占位封面之外，磁带本身分**内部结构**（`tape_inner`：卷盘承座、导带柱）
  与**外壳**（`tape_shell`）两层资源 —— 外壳用透明或半透明贴图就能透出内部卷盘与磁带，不透明贴图则只需在观察窗处挖穿；
  卷芯还能用 `reel_hub` 贴图替换。按钮默认由程序画成复古塑料键帽（也可整图覆盖）。
  `skins\default`（透明塑料壳）与 `skins\yellow90`（经典黄黑壳）是两套现成样板，
  皮肤之间可 `base_pack` 继承；缺项自动回退内置矢量绘制，任何不完整的皮肤包都能安全运行。
